...
docker-build:  ## Build docker image with the manager.
	docker buildx build --platform=linux/arm64,linux/amd64 --build-arg "builddate_arg=$(BUILDDATE)" --build-arg "version_arg=$(BUILD_VERSION)" $(CONTAINERBUILDTAGS) -t ${IMG} -t ${IMG}:latest --push .
